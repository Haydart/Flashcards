...
5. ~~Firestore schema for storing sanitized transcript + grade history per card/session~~ — resolved by ADR-0014: the sanitized transcript is written to the session document's embedded `outcomes` map, under the answered card's id in the single session-commit batch. Session-state tracking and the read-only revisit UI for already-answered cards remain undesigned.

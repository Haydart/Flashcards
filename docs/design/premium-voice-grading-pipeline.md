...
`AudioRecord` with `VOICE_RECOGNITION` audio source, 16kHz mono — raw PCM frames are needed for real-time VAD analysis, which rules out `MediaRecorder` (writes encoded files, not raw frames).

Bluetooth earphones require explicitly starting Bluetooth SCO (`AudioManager.startBluetoothSco()`) to route mic input through the BT HFP profile. This adds setup latency and forces narrowband sample rates (8/16kHz), but is the only reliable path for BT Classic headsets. Modern LE Audio/LC3 (Android 13+) would give better quality/latency but only covers LE Audio earphones — excluding most current Bluetooth Classic headsets — so it would just become a second code path on top of SCO, not a replacement. Wired earphones need no special handling.
...
- **Raw capture playback**: record a short clip via `AudioRecord`, play it back unmodified — confirms capture + (if relevant) BT SCO routing works before any DSP is involved.

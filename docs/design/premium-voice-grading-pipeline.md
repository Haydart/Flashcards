...
7. ~~`checkEntitlement()` and other non-audio `VoiceGradingApi` members: migrate to callable functions for consistency with the now-`onCall`-based grading endpoint, or leave as plain REST since they don't need streaming~~ — resolved during implementation: left as plain REST via `VoiceGradingRetrofitService`/`RealVoiceGradingApi`. None of the three (`transcribe`, `sanitizeAndGrade`, `checkEntitlement`) return more than one response, so streaming buys nothing for them, and migrating would mean re-authoring their debug-screen call sites for no behavior change.
8. ~~`VoiceGradingApi`/`VoiceAnswerGradingRepository`/`GradeSpokenAnswerUseCase`/`VoiceAnswerController`/`FakeVoiceGradingApi` file-level changes needed to actually carry the two-phase result through the app~~ — implemented: `gradeVoiceAnswer` returns `Flow<VoiceGradingStreamEventDto>` (data) / `Flow<VoiceAnswerGradingEvent>` (domain), collected in `VoiceAnswerController.gradeUtterance()` and written into a new `VoiceAnswerState.sanitizedTranscript` field ahead of `lastGrade`. `RetrofitVoiceGradingApi` was renamed `RealVoiceGradingApi` (per this doc's own naming, above) and now holds both the streaming `FirebaseFunctions` client for `gradeVoiceAnswer` and the Retrofit service for the three REST-only members. `FakeVoiceGradingApi` emits a `TranscriptChunk` after a simulated upload+STT+sanitize delay, then a `Graded` after a second simulated grading-LLM delay, so the debug screen and UI exercise the same two-window latency shape the real streamed call has.

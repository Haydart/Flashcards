...
5. ~~Firestore schema for storing sanitized transcript + grade history per card/session~~ — resolved by ADR-0014: the sanitized transcript is written to `users/{uid}/sessions/{sessionId}/outcomes/{cardId}.transcript` in the single session-commit batch. Session-state tracking and the read-only revisit UI for already-answered cards remain undesigned.

...
- `color` is a mandatory hex-string field on every Category document (e.g. `"#6B2FA0"`), and `iconUrl` (previously an unused always-empty field) is now a mandatory Firebase Storage URL to a single-color SVG glyph, tinted at render time using `color`. Neither field exists on Subcategory documents — icon+color are Category-level only. See `docs/design/category-icon-color.md` for the full rendering design and rationale.

...
- `curationRequests/{cardId}` (ADR-0017) and per-user `cardProgress/{cardId}` are unaffected — both are keyed by `cardId` in collections that never depended on where the card document itself lives.

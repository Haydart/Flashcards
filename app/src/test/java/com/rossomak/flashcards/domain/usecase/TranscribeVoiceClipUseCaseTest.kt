...
class TranscribeVoiceClipUseCaseTest {

    private val voiceAnswerGradingRepository: VoiceAnswerGradingRepository = mockk()
    private val useCase = TranscribeVoiceClipUseCase(voiceAnswerGradingRepository)

    @Test
    fun `forwards wav bytes to repository and returns the transcript`() = runTest {
        val wavBytes = byteArrayOf(1, 2, 3)
        val transcript = "a mechanism keeping the cpu awake"
        coEvery { voiceAnswerGradingRepository.transcribe(wavBytes) } returns Result.success(transcript)

        val result = useCase(wavBytes)

        result.getOrNull() shouldBe transcript
        coVerify(exactly = 1) { voiceAnswerGradingRepository.transcribe(wavBytes) }
    }
    ...
    fun `forwards failure result unchanged`() = runTest {
        ...
        coEvery { voiceAnswerGradingRepository.transcribe(wavBytes) } returns Result.failure(error)

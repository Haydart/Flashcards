...
**The persisted document is a distinct, larger shape**, not this type verbatim: the server-authoritative `submitStudySession` Cloud Function ([ADR-0014](docs/adr/0014-session-stats-written-at-summary-screen.md), spec 08) builds it from the submitted Session Result plus the computed `xpBreakdown`/`xpTotal` and streak/daily-goal awards ([ADR-0047](docs/adr/0047-xp-values-behind-a-config-repository.md), [ADR-0048](docs/adr/0048-streak-and-daily-goal-ride-the-session-payload.md)), and that document is what's written once, inside one Firestore transaction — nothing is written while a session runs, and the client no longer writes any of it directly. Stored at `users/{uid}/sessions/{sessionId}`, with `studyMode` as the one field a reader needs to know which shape the rest of the document is in.
